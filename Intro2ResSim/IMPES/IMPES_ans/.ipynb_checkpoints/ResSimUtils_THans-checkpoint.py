...
def cal_Ta_ans(perm_x, perm_rel_a, h, dx, dy, vis_a, FVF_a, Pressure, nx, BC_w=0, BC_e= 0, Pb_w = 0, Pb_e = 0, Perm_w = 0, Perm_e = 0, RelPerm = 0):
    # ----- Input ----- #
        ...
        if i == 0:
            if BC_w != 1:
                ...
                if Pressure[i] < Pb_w:
                    ...
                    T_wbc = (perm_rel_a[i] + RelPerm)/2 * Tw;
                T_o[i]    = T_wbc
                bcs_a[i]  = T_wbc*Pb_w
                T_west[i] = 0
    ...
    for i in range(0, nx):
        if i == nx-1:
            if BC_e != 1:
                ...
                Tw = h*dy*(2*(perm_x[i] * Perm_e )/(perm_x[i] + Perm_e) )/(vis_a*dx*FVF_a)
                if Pressure[i] < Pb_e:
                    T_ebc = RelPerm * Tw;
                elif Pressure[i] > Pb_e:
                    T_ebc = perm_rel_a[i] * Tw;
                else:
                    T_ebc = (perm_rel_a[i] + RelPerm)/2 * Tw;
                T_o[i]    = T_ebc
                bcs_a[i]  = T_ebc*Pb_e
                T_east[i] = 0
        else:
